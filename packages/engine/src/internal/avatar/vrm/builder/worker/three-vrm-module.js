...
class VRMExpression extends Object3D {
    /**
    ...
    applyWeight(options) {
        var _a;
        let actualWeight = this.isBinary
            ? this.weight <= 0.5
                ? 0.0
                : 1.0
            : this.weight;
        actualWeight *=
            (_a =
                options === null || options === void 0
                    ? void 0
                    : options.multiplier) !== null && _a !== void 0
                ? _a
                : 1.0;
...
function extractPrimitivesInternal(gltf, nodeIndex, node) {
     ...
     *
     * - `Mesh`: The only primitive of the mesh *
     *
     * ### A node with a (mesh, multiple primitives)
     *
     * - `Group`: The root of the mesh
     *   - `Mesh`: A primitive of the mesh *
     *   - `Mesh`: A primitive of the mesh (2) *
     *
     ...
     *   - `Mesh`: A primitive of the mesh *
     *   - `Mesh`: A primitive of the mesh (2) *
     *   - `Mesh`: A primitive of a MESH OF THE CHILD
     *
     ...
     *   - `Mesh`: A primitive of the mesh *
     *   - `Mesh`: A primitive of the mesh (2) *
     *   - `Group`: The root of a MESH OF THE CHILD
     *     - `Mesh`: A primitive of the mesh of the child
     *     - `Mesh`: A primitive of the mesh of the child (2)
     *
     * ### A node with a (mesh, multiple primitives) BUT the node is a bone
     *
     * - `Bone`: The root of the node, as a bone
     *   - `Group`: The root of the mesh
     *     - `Mesh`: A primitive of the mesh *
     *     - `Mesh`: A primitive of the mesh (2) *
     *
    ...
    const schemaNode =
        (_a = json.nodes) === null || _a === void 0 ? void 0 : _a[nodeIndex];
    if (schemaNode == null) {
        console.warn(
            `extractPrimitivesInternal: Attempt to use nodes[${nodeIndex}] of glTF but the node doesn't exist`
        );
        return null;
    }
    ...
    const schemaMesh =
        (_b = json.meshes) === null || _b === void 0 ? void 0 : _b[meshIndex];
    if (schemaMesh == null) {
        console.warn(
            `extractPrimitivesInternal: Attempt to use meshes[${meshIndex}] of glTF but the mesh doesn't exist`
        );
        return null;
    }
...
/**
 * Extract primitives ( `Mesh[]` ) of a node from a loaded GLTF.
 * The main purpose of this function is to distinguish primitives and children from a node that has both meshes and children.
 *
 * It utilizes the behavior that GLTFLoader adds mesh primitives to the node object ( `Group` ) first then adds its children.
 *
...
/**
 * Extract primitives ( `Mesh[]` ) of nodes from a loaded GLTF.
...
function gltfGetAssociatedMaterialIndex(parser, material) {
    ...
    if (threeRevision >= 133) {
        index =
            (_b =
                (_a = parser.associations.get(material)) === null ||
                _a === void 0
                    ? void 0
                    : _a.materials) !== null && _b !== void 0
                ? _b
                : null;
    } else {
...
class VRMExpressionManager {
    ...
    getExpression(name) {
        var _a;
        return (_a = this._expressionMap[name]) !== null && _a !== void 0
            ? _a
            : null;
    }
    /**
    ...
    unregisterExpression(expression) {
        ...
        if (index === -1) {
            console.warn(
                "VRMExpressionManager: The specified expressions is not registered"
            );
        }
    ...
    getValue(name) {
        ...
        return (_a =
            expression === null || expression === void 0
                ? void 0
                : expression.weight) !== null && _a !== void 0
            ? _a
            : null;
    }
    /**
...
class VRMExpressionMaterialColorBind {
    constructor({ material, type, targetValue, targetAlpha }) {
        ...
        this.targetAlpha =
            targetAlpha !== null && targetAlpha !== void 0 ? targetAlpha : 1.0;
        // init bind state
    ...
    _initColorBindState() {
        ...
        const propertyName =
            (_b =
                (_a =
                    propertyNameMap === null || propertyNameMap === void 0
                        ? void 0
                        : propertyNameMap[type]) === null || _a === void 0
                    ? void 0
                    : _a[0]) !== null && _b !== void 0
                ? _b
                : null;
        if (propertyName == null) {
            console.warn(
                `Tried to add a material color bind to the material ${
                    (_c = material.name) !== null && _c !== void 0
                        ? _c
                        : "(no name)"
...
class VRMExpressionMorphTargetBind {
    ...
    applyWeight(weight) {
        this.primitives.forEach((mesh) => {
            var _a;
            if (
                ((_a = mesh.morphTargetInfluences) === null || _a === void 0
                    ? void 0
                    : _a[this.index]) != null
            ) {
...
class VRMExpressionTextureTransformBind {
        ...
        if (propertyNames == null) {
            console.warn(
                `Tried to add a texture transform bind to the material ${
                    (_b = material.name) !== null && _b !== void 0
                        ? _b
                        : "(no name)"
            ...
            propertyNames.forEach((propertyName) => {
                var _a;
                const texture =
                    (_a = material[propertyName]) === null || _a === void 0
                        ? void 0
                        : _a.clone();
                if (!texture) {
                    return null;
    ...
    applyWeight(weight) {
        this._properties.forEach((property) => {
            ...
            target.offset.add(
                _v2.copy(property.deltaOffset).multiplyScalar(weight)
            );
            target.repeat.add(
                _v2.copy(property.deltaScale).multiplyScalar(weight)
            );
...
class VRMExpressionLoaderPlugin {
    ...
    _v1Import(gltf) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            const isVRMUsed =
                ((_a = json.extensionsUsed) === null || _a === void 0
                    ? void 0
                    : _a.indexOf("VRMC_vrm")) !== -1;
            if (!isVRMUsed) {
                return null;
            }
            const extension =
                (_b = json.extensions) === null || _b === void 0
                    ? void 0
                    : _b["VRMC_vrm"];
            if (!extension) {
                return null;
            ...
            if (!POSSIBLE_SPEC_VERSIONS$4.has(specVersion)) {
                console.warn(
                    `VRMExpressionLoaderPlugin: Unknown VRMC_vrm specVersion "${specVersion}"`
                );
                return null;
            }
            ...
            if (schemaExpressions.preset != null) {
                Object.entries(schemaExpressions.preset).forEach(
            ...
            if (schemaExpressions.custom != null) {
                Object.entries(schemaExpressions.custom).forEach(
                    ([name, schemaExpression]) => {
                        if (presetNameSet.has(name)) {
                            console.warn(
            ...
            // load expressions
            yield Promise.all(
                Array.from(nameSchemaExpressionMap.entries()).map(
                    ([name, schemaExpression]) =>
                        __awaiter$6(this, void 0, void 0, function* () {
                            ...
                            expression.isBinary =
                                (_c = schemaExpression.isBinary) !== null &&
                                _c !== void 0
                                    ? _c
                                    : false;
                            expression.overrideBlink =
                                (_d = schemaExpression.overrideBlink) !==
                                    null && _d !== void 0
                                    ? _d
                                    : "none";
                            expression.overrideLookAt =
                                (_e = schemaExpression.overrideLookAt) !==
                                    null && _e !== void 0
                                    ? _e
                                    : "none";
                            expression.overrideMouth =
                                (_f = schemaExpression.overrideMouth) !==
                                    null && _f !== void 0
                                    ? _f
                                    : "none";
                            (_g = schemaExpression.morphTargetBinds) === null ||
                            _g === void 0
                                ? void 0
                                : _g.forEach((bind) =>
                                      __awaiter$6(
                            ...
                            if (
                                schemaExpression.materialColorBinds ||
                                schemaExpression.textureTransformBinds
                            ) {
                                ...
                                (_h = schemaExpression.materialColorBinds) ===
                                    null || _h === void 0
                                    ? void 0
                                    : _h.forEach((bind) =>
                                          __awaiter$6(
                                              ...
                                              function* () {
                                                  const materials =
                                                      gltfMaterials.filter(
                                                          (material) => {
                                                              const materialIndex =
                                                                  gltfGetAssociatedMaterialIndex(
                                                                      this
                                                                          .parser,
                                                                      material
                                                                  );
    ...
    _v0Import(gltf) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            // early abort if it doesn't use vrm
            const vrmExt =
                (_a = json.extensions) === null || _a === void 0
                    ? void 0
                    : _a.VRM;
            if (!vrmExt) {
                return null;
                ...
                schemaBlendShapeGroups.map((schemaGroup) =>
                    __awaiter$6(this, void 0, void 0, function* () {
                        ...
                        const v1PresetName =
                            ...
                            null;
                        const name =
                            v1PresetName !== null && v1PresetName !== void 0
                                ? v1PresetName
                                : schemaGroup.name;
                        if (name == null) {
                            console.warn(
                                                        ...
                                                        new VRMExpressionMorphTargetBind(
                                                            {
                                                                ...
                                                                weight:
                                                                    ...
                                                                    _d !==
                                                                        void 0
                                                                        ? _d
                                                                        : 100), // narrowing the range from [ 0.0 - 100.0 ] to [ 0.0 - 1.0 ]
                                                            }
                                                        )
                                                    );
                                                }
                        ...
                        if (materialValues && materialValues.length !== 0) {
                            materialValues.forEach((materialValue) => {
                                ...
                                materials.forEach((material) => {
                                    ...
                                    if (
                                        materialPropertyName === "_MainTex_ST"
                                    ) {
                                        const scale = new Vector2(
                                            materialValue.targetValue[0],
                                            materialValue.targetValue[1]
                                        );
                                        const offset = new Vector2(
                                            materialValue.targetValue[2],
                                            materialValue.targetValue[3]
                                        );
...
class VRMFirstPerson {
    ...
    copy(source) {
        if (this.humanoid !== source.humanoid) {
            throw new Error(
                "VRMFirstPerson: humanoid must be same in order to copy"
            );
        }
     ...
     */
    clone() {
        return new VRMFirstPerson(this.humanoid, this.meshAnnotations).copy(
            this
        );
    }
    /**
        ...
        this.meshAnnotations.forEach((item) => {
            item.meshes.forEach((mesh) => {
                if (item.type === "firstPersonOnly") {
                    ...
                    mesh.traverse((child) =>
                        child.layers.set(this._firstPersonOnlyLayer)
                    );
                } else if (item.type === "thirdPersonOnly") {
                    mesh.layers.set(this._thirdPersonOnlyLayer);
                    mesh.traverse((child) =>
                        child.layers.set(this._thirdPersonOnlyLayer)
                    );
    ...
    _createErasedMesh(src, erasingBonesIndex) {
        ...
        const skinIndexAttrArray =
            skinIndexAttr instanceof GLBufferAttribute
                ? []
                : skinIndexAttr.array;
        const skinIndex = [];
        ...
        const skinWeightAttrArray =
            skinWeightAttr instanceof GLBufferAttribute
                ? []
...
class VRMFirstPersonLoaderPlugin {
    ...
    afterRoot(gltf) {
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            gltf.userData.vrmFirstPerson = yield this._import(
                gltf,
                vrmHumanoid
            );
        });
    ...
    _v1Import(gltf, humanoid) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (!POSSIBLE_SPEC_VERSIONS$3.has(specVersion)) {
                console.warn(
                    `VRMFirstPersonLoaderPlugin: Unknown VRMC_vrm specVersion "${specVersion}"`
                );
                return null;
            }
            ...
            const nodePrimitivesMap =
                yield gltfExtractPrimitivesFromNodes(gltf);
                        ...
                        type:
                            (_a =
                                annotation === null || annotation === void 0
                                    ? void 0
                                    : annotation.type) !== null && _a !== void 0
                                ? _a
                                : "both",
                    });
                }
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            Array.from(nodePrimitivesMap.entries()).forEach(
                ([nodeIndex, primitives]) => {
                    ...
                    meshAnnotations.push({
                        meshes: primitives,
                        type: this._convertV0FlagToV1Type(
...
const _v3A$4$1 = new Vector3();
const _v3B$2$1 = new Vector3();
const _quatA$5 = new Quaternion();
class VRMHumanoidHelper extends Group {
    constructor(humanoid) {
        super();
        this.vrmHumanoid = humanoid;
        this._boneAxesMap = new Map();
        Object.values(humanoid.humanBones).forEach((bone) => {
            const helper = new AxesHelper(1.0);
...
const _v3A$3$2 = new Vector3();
const _quatA$4 = new Quaternion();
/**
            ...
            if (state === null || state === void 0 ? void 0 : state.rotation) {
                ...
                if (restState.rotation) {
                    node.quaternion.multiply(
                        _quatA$4.fromArray(restState.rotation)
                    );
                }
            }
        });
...
const _v3A$2$2 = new Vector3();
const _quatA$3$1 = new Quaternion();
...
class VRMHumanoidRig extends VRMRig {
    static _setupTransforms(modelRig) {
        ...
        const parentInverseWorldRotations = {};
        VRMHumanBoneList.forEach((boneName) => {
            ...
            if (boneNode) {
                ...
                boneNode.matrixWorld.decompose(
                    boneWorldPosition,
                    boneWorldRotation,
                    _v3A$2$2
                );
                ...
                (_a = boneNode.parent) === null || _a === void 0
                    ? void 0
                    : _a.matrixWorld.decompose(
                          _v3A$2$2,
                          parentWorldRotation,
                          _v3A$2$2
                      );
                parentWorldRotations[boneName] = parentWorldRotation;
                parentInverseWorldRotations[boneName] = quatInvertCompat$1(
                    parentWorldRotation.clone()
                );

                parentWorldRotations[boneName].isIdentity =
                    isIdentityQuaternion(parentWorldRotations[boneName]);
                parentInverseWorldRotations[boneName].isIdentity =
                    isIdentityQuaternion(parentInverseWorldRotations[boneName]);
            }
        });
        ...
        VRMHumanBoneList.forEach((boneName) => {
            ...
            if (boneNode) {
                ...
                while (parentBoneWorldPosition == null) {
                    ...
                    parentBoneWorldPosition =
                        boneWorldPositions[currentBoneName];
                }
                // add to hierarchy
                const rigBoneNode = new Object3D();
                rigBoneNode.name = "Normalized_" + boneNode.name;
                const parentRigBoneNode = currentBoneName
                    ? (_a = rigBones[currentBoneName]) === null || _a === void 0
                        ? void 0
                        : _a.node
                    : root;
    ...
    constructor(humanoid) {
        const {
            ...
            boneRotations,
            parentInverseWorldRotations,
        } = VRMHumanoidRig._setupTransforms(humanoid);
        super(rigBones);
        this.original = humanoid;
        this.root = root;
        this._parentWorldRotations = parentWorldRotations;
        this._parentInverseWorldRotations = parentInverseWorldRotations;
    ...
    update() {
        ...
        let cc = 0;
        const boneCount = VRMHumanBoneList.length;

        while (index < boneCount) {
            const boneName = VRMHumanBoneList[index];

            const boneNode = this.original.getBoneNode(boneName);
            if (boneNode != null) {
                const rigBoneNode = this.getBoneNode(boneName);
                const parentWorldRotation =
                    this._parentWorldRotations[boneName];
                const invParentWorldRotation =
                    this._parentInverseWorldRotations[boneName];
                const boneRotation = this._boneRotations[boneName];

                boneNode.quaternion.copy(rigBoneNode.quaternion);

                if (parentWorldRotation.isIdentity == false) {
                    boneNode.quaternion
                ...
                boneNode.quaternion.multiply(boneRotation);

                // Move the mass center of the VRM
                if (boneName === "hips") {
                    const boneWorldPosition =
                        rigBoneNode.getWorldPosition(_boneWorldPos);
                    boneNode.parent?.updateWorldMatrix(true, false);
                    const parentWorldMatrix = boneNode.parent?.matrixWorld;
                    const localPosition = boneWorldPosition.applyMatrix4(
                        tmpMat.copy(parentWorldMatrix).invert()
                    );
...
class VRMHumanoid {
    ...
    get restPose() {
        console.warn(
            "VRMHumanoid: restPose is deprecated. Use either rawRestPose or normalizedRestPose instead."
        );
    ...
    constructor(humanBones, options) {
        var _a;
        this.autoUpdateHumanBones =
            (_a =
                options === null || options === void 0
                    ? void 0
                    : options.autoUpdateHumanBones) !== null && _a !== void 0
                ? _a
                : true;
    ...
    getPose() {
        console.warn(
            "VRMHumanoid: getPose() is deprecated. Use either getRawPose() or getNormalizedPose() instead."
        );
    ...
    setPose(poseObject) {
        console.warn(
            "VRMHumanoid: setPose() is deprecated. Use either setRawPose() or setNormalizedPose() instead."
        );
    ...
    resetPose() {
        console.warn(
            "VRMHumanoid: resetPose() is deprecated. Use either resetRawPose() or resetNormalizedPose() instead."
        );
    ...
    getBone(name) {
        console.warn(
            "VRMHumanoid: getBone() is deprecated. Use either getRawBone() or getNormalizedBone() instead."
        );
...
class VRMHumanoidLoaderPlugin {
    ...
    _v1Import(gltf) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (!POSSIBLE_SPEC_VERSIONS$2$1.has(specVersion)) {
                console.warn(
                    `VRMHumanoidLoaderPlugin: Unknown VRMC_vrm specVersion "${specVersion}"`
                );
                return null;
            }
            ...
            const existsPreviousThumbName =
                schemaHumanoid.humanBones.leftThumbIntermediate != null ||
                schemaHumanoid.humanBones.rightThumbIntermediate != null;
            const humanBones = {};
            if (schemaHumanoid.humanBones != null) {
                yield Promise.all(
                    Object.entries(schemaHumanoid.humanBones).map(
                        ([boneNameString, schemaHumanBone]) =>
                            __awaiter$6(this, void 0, void 0, function* () {
                                ...
                                if (existsPreviousThumbName) {
                                    const thumbBoneName =
                                        thumbBoneNameMap[boneName];
                                    if (thumbBoneName != null) {
                                        boneName = thumbBoneName;
                                    }
                                }
                                const node = yield this.parser.getDependency(
                                    "node",
                                    index
                                );
            ...
            const humanoid = new VRMHumanoid(
                this._ensureRequiredBonesExist(humanBones),
                {
                    autoUpdateHumanBones: this.autoUpdateHumanBones,
                }
            );
    ...
    _ensureRequiredBonesExist(humanBones) {
        ...
        const missingRequiredBones = Object.values(
            VRMRequiredHumanBoneName
        ).filter((requiredBoneName) => humanBones[requiredBoneName] == null);
...
class FanBufferGeometry extends BufferGeometry {
    constructor() {
        super();
    ...
    _buildPosition() {
        ...
        for (let i = 0; i < 64; i++) {
            ...
            this._attrPos.setXYZ(
                i + 1,
                this._currentRadius * Math.sin(t),
                0.0,
                this._currentRadius * Math.cos(t)
            );
        }
...
class LineAndSphereBufferGeometry extends BufferGeometry {
    constructor() {
        super();
        this.radius = 0.0;
        this._currentRadius = 0.0;
...
const _quatA$2$1 = new Quaternion();
const _quatB$2$1 = new Quaternion();
const _v3A$1$2 = new Vector3();
const _v3B$1$2 = new Vector3();
const SQRT_2_OVER_2 = Math.sqrt(2.0) / 2.0;
const QUAT_XY_CW90 = new Quaternion(0, 0, -SQRT_2_OVER_2, SQRT_2_OVER_2);
const VEC3_POSITIVE_Y = new Vector3(0.0, 1.0, 0.0);
class VRMLookAtHelper extends Group {
    constructor(lookAt) {
        super();
    ...
    updateMatrixWorld(force) {
        ...
        this._meshPitch.quaternion.multiply(
            _quatB$2$1.setFromAxisAngle(VEC3_POSITIVE_Y, yaw)
        );
...
const VEC3_POSITIVE_Z$1 = new Vector3(0.0, 0.0, 1.0);
const _v3A$6 = new Vector3();
const _v3B$3 = new Vector3();
const _v3C$2 = new Vector3();
const _quatA$1$1 = new Quaternion();
const _quatB$1$1 = new Quaternion();
const _quatC$1 = new Quaternion();
const _quatD = new Quaternion();
const _eulerA$1 = new Euler();
/**
...
class VRMLookAt {
    ...
    constructor(humanoid, applier) {
         ...
         */
        this.faceFront = new Vector3(0.0, 0.0, 1.0);
        this.humanoid = humanoid;
        this.applier = applier;
        this._yaw = 0.0;
        this._pitch = 0.0;
        this._needsUpdate = true;
        this._restHeadWorldQuaternion = this.getLookAtWorldQuaternion(
            new Quaternion()
        );
    }
    /**
    ...
    getEuler(target) {
        return target.set(
            MathUtils.DEG2RAD * this._pitch,
            MathUtils.DEG2RAD * this._yaw,
            0.0,
            "YXZ"
        );
    }
    ...
    copy(source) {
        if (this.humanoid !== source.humanoid) {
            throw new Error(
                "VRMLookAt: humanoid must be same in order to copy"
            );
        }
    ...
    getFaceFrontQuaternion(target) {
        ...
        const [faceFrontAzimuth, faceFrontAltitude] = calcAzimuthAltitude(
            this.faceFront
        );
        _eulerA$1.set(
            0.0,
            0.5 * Math.PI + faceFrontAzimuth,
            faceFrontAltitude,
            "YZX"
        );
        return target
            .setFromEuler(_eulerA$1)
            .premultiply(_quatD.copy(this._restHeadWorldQuaternion).invert());
    }
    /**
    ...
    lookAt(position) {
        ...
        const headRotDiffInv = _quatA$1$1
            ...
            .multiply(
                quatInvertCompat$1(this.getLookAtWorldQuaternion(_quatB$1$1))
            );
        const headPos = this.getLookAtWorldPosition(_v3B$3);
        const lookAtDir = _v3C$2
            .copy(position)
            .sub(headPos)
            .applyQuaternion(headRotDiffInv)
            .normalize();
        // calculate angles
...
const _quatA$6 = new Quaternion();
const _quatB$3 = new Quaternion();
const _eulerA = new Euler(0.0, 0.0, 0.0, "YXZ");
/**
...
class VRMLookAtBoneApplier {
    ...
    constructor(
        ...
        if (leftEye) {
            ...
            getWorldQuaternionLite(
                leftEye.parent,
                this._restLeftEyeParentWorldQuat
            );
        }
        if (rightEye) {
    ...
    applyYawPitch(yaw, pitch) {
        const leftEye = this.humanoid.getRawBoneNode("leftEye");
        const rightEye = this.humanoid.getRawBoneNode("rightEye");
        const leftEyeNormalized =
            this.humanoid.getNormalizedBoneNode("leftEye");
        const rightEyeNormalized =
            this.humanoid.getNormalizedBoneNode("rightEye");
        // left
        if (leftEye) {
            if (pitch < 0.0) {
                _eulerA.x =
                    -MathUtils.DEG2RAD * this.rangeMapVerticalDown.map(-pitch);
            } else {
                _eulerA.x =
                    MathUtils.DEG2RAD * this.rangeMapVerticalUp.map(pitch);
            }
            if (yaw < 0.0) {
                _eulerA.y =
                    -MathUtils.DEG2RAD * this.rangeMapHorizontalInner.map(-yaw);
            } else {
                _eulerA.y =
                    MathUtils.DEG2RAD * this.rangeMapHorizontalOuter.map(yaw);
            }
            ...
            leftEyeNormalized.quaternion
                .copy(_quatB$3)
                .multiply(_quatA$6)
                .multiply(_quatB$3.invert());
    ...
    lookAt(euler) {
        console.warn(
            "VRMLookAtBoneApplier: lookAt() is deprecated. use apply() instead."
        );
...
class VRMLookAtExpressionApplier {
    ...
    applyYawPitch(yaw, pitch) {
        if (pitch < 0.0) {
            ...
            this.expressions.setValue(
                "lookUp",
                this.rangeMapVerticalUp.map(-pitch)
            );
        } else {
            this.expressions.setValue("lookUp", 0.0);
            this.expressions.setValue(
                "lookDown",
                this.rangeMapVerticalDown.map(pitch)
            );
        }
        if (yaw < 0.0) {
            ...
            this.expressions.setValue(
                "lookRight",
                this.rangeMapHorizontalOuter.map(-yaw)
            );
        } else {
...
class VRMLookAtLoaderPlugin {
    ...
    afterRoot(gltf) {
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (vrmHumanoid === null) {
                ...
                throw new Error(
                    "VRMLookAtLoaderPlugin: vrmHumanoid is undefined. VRMHumanoidLoaderPlugin have to be used first"
                );
            }
            ...
            gltf.userData.vrmLookAt = yield this._import(
                gltf,
                vrmHumanoid,
    ...
    _v1Import(gltf, humanoid, expressions) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (!POSSIBLE_SPEC_VERSIONS$1$1.has(specVersion)) {
                console.warn(
                    `VRMLookAtLoaderPlugin: Unknown VRMC_vrm specVersion "${specVersion}"`
                );
                return null;
            }
            ...
            const defaultOutputScale =
                schemaLookAt.type === "expression" ? 1.0 : 10.0;
            const mapHI = this._v1ImportRangeMap(
                schemaLookAt.rangeMapHorizontalInner,
                defaultOutputScale
            );
            const mapHO = this._v1ImportRangeMap(
                schemaLookAt.rangeMapHorizontalOuter,
                defaultOutputScale
            );
            const mapVD = this._v1ImportRangeMap(
                schemaLookAt.rangeMapVerticalDown,
                defaultOutputScale
            );
            const mapVU = this._v1ImportRangeMap(
                schemaLookAt.rangeMapVerticalUp,
                defaultOutputScale
            );
            let applier;
            if (schemaLookAt.type === "expression") {
                applier = new VRMLookAtExpressionApplier(
                    expressions,
                    mapHI,
                    mapHO,
                    mapVD,
            ...
            lookAt.offsetFromHeadBone.fromArray(
                (_c = schemaLookAt.offsetFromHeadBone) !== null && _c !== void 0
                    ? _c
                    : [0.0, 0.06, 0.0]
            );
            return lookAt;
    ...
    _v1ImportRangeMap(schemaRangeMap, defaultOutputScale) {
        ...
        let inputMaxValue =
            (_a =
                schemaRangeMap === null || schemaRangeMap === void 0
                    ? void 0
                    : schemaRangeMap.inputMaxValue) !== null && _a !== void 0
                ? _a
                : 90.0;
        const outputScale =
            (_b =
                schemaRangeMap === null || schemaRangeMap === void 0
                    ? void 0
                    : schemaRangeMap.outputScale) !== null && _b !== void 0
                ? _b
                : defaultOutputScale;
    ...
    _v0Import(gltf, humanoid, expressions) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            const defaultOutputScale =
                schemaFirstPerson.lookAtTypeName === "BlendShape" ? 1.0 : 10.0;
            const mapHI = this._v0ImportDegreeMap(
                schemaFirstPerson.lookAtHorizontalInner,
                defaultOutputScale
            );
            const mapHO = this._v0ImportDegreeMap(
                schemaFirstPerson.lookAtHorizontalOuter,
                defaultOutputScale
            );
            const mapVD = this._v0ImportDegreeMap(
                schemaFirstPerson.lookAtVerticalDown,
                defaultOutputScale
            );
            const mapVU = this._v0ImportDegreeMap(
                schemaFirstPerson.lookAtVerticalUp,
                defaultOutputScale
            );
            let applier;
            ...
            if (schemaFirstPerson.firstPersonBoneOffset) {
                lookAt.offsetFromHeadBone.set(
                    (_b = schemaFirstPerson.firstPersonBoneOffset.x) !== null &&
                        _b !== void 0
                        ? _b
                        : 0.0,
                    (_c = schemaFirstPerson.firstPersonBoneOffset.y) !== null &&
                        _c !== void 0
                        ? _c
                        : 0.06,
                    -((_d = schemaFirstPerson.firstPersonBoneOffset.z) !==
                        null && _d !== void 0
                        ? _d
                        : 0.0)
                );
    ...
    _v0ImportDegreeMap(schemaDegreeMap, defaultOutputScale) {
        var _a, _b;
        const curve =
            schemaDegreeMap === null || schemaDegreeMap === void 0
                ? void 0
                : schemaDegreeMap.curve;
        ...
        let xRange =
            (_a =
                schemaDegreeMap === null || schemaDegreeMap === void 0
                    ? void 0
                    : schemaDegreeMap.xRange) !== null && _a !== void 0
                ? _a
                : 90.0;
        const yRange =
            (_b =
                schemaDegreeMap === null || schemaDegreeMap === void 0
                    ? void 0
                    : schemaDegreeMap.yRange) !== null && _b !== void 0
                ? _b
                : defaultOutputScale;
        ...
        if (xRange < INPUT_MAX_VALUE_MINIMUM) {
            console.warn(
                "VRMLookAtLoaderPlugin: xRange of a degree map is too small. Consider reviewing the degree map!"
            );
...
class VRMMetaLoaderPlugin {
    ...
    constructor(parser, options) {
        var _a, _b, _c;
        this.parser = parser;
        this.needThumbnailImage =
            (_a =
                options === null || options === void 0
                    ? void 0
                    : options.needThumbnailImage) !== null && _a !== void 0
                ? _a
                : true;
        this.acceptLicenseUrls =
            (_b =
                options === null || options === void 0
                    ? void 0
                    : options.acceptLicenseUrls) !== null && _b !== void 0
                ? _b
        ...
        this.acceptV0Meta =
            (_c =
                options === null || options === void 0
                    ? void 0
                    : options.acceptV0Meta) !== null && _c !== void 0
                ? _c
                : true;
    }
    afterRoot(gltf) {
    ...
    _v1Import(gltf) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (!POSSIBLE_SPEC_VERSIONS$5.has(specVersion)) {
                console.warn(
                    `VRMMetaLoaderPlugin: Unknown VRMC_vrm specVersion "${specVersion}"`
                );
                return null;
            }
            ...
            if (!acceptLicenseUrlsSet.has(licenseUrl)) {
                throw new Error(
                    `VRMMetaLoaderPlugin: The license url "${licenseUrl}" is not accepted`
                );
            }
            let thumbnailImage = undefined;
            if (this.needThumbnailImage && schemaMeta.thumbnailImage != null) {
                thumbnailImage =
                    (_c = yield this._extractGLTFImage(
                        schemaMeta.thumbnailImage
                    )) !== null && _c !== void 0
                        ? _c
                        : undefined;
            }
            return {
                ...
                allowExcessivelyViolentUsage:
                    schemaMeta.allowExcessivelyViolentUsage,
                allowExcessivelySexualUsage:
                    schemaMeta.allowExcessivelySexualUsage,
                commercialUsage: schemaMeta.commercialUsage,
                allowPoliticalOrReligiousUsage:
                    schemaMeta.allowPoliticalOrReligiousUsage,
                allowAntisocialOrHateUsage:
                    schemaMeta.allowAntisocialOrHateUsage,
    ...
    _v0Import(gltf) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (!this.acceptV0Meta) {
                throw new Error(
                    "VRMMetaLoaderPlugin: Attempted to load VRM0.0 meta but acceptV0Meta is false"
                );
            }
            ...
            if (
                ...
                texture = yield this.parser.getDependency(
                    "texture",
                    schemaMeta.texture
                );
            }
    ...
    _extractGLTFImage(index) {
        ...
        return __awaiter$6(this, void 0, void 0, function* () {
            ...
            if (source.bufferView != null) {
                const bufferView = yield this.parser.getDependency(
                    "bufferView",
                    source.bufferView
                );
            ...
            return yield loader
                .loadAsync(resolveURL(sourceURI, this.parser.options.path))
                .catch((error) => {
                    console.error(error);
                    console.warn(
                        "VRMMetaLoaderPlugin: Failed to load a thumbnail image"
                    );
                    return null;
                });
        });
    ...
    constructor(parser, options) {
        ...
        const helperRoot =
            options === null || options === void 0
                ? void 0
                : options.helperRoot;
        const autoUpdateHumanBones =
            options === null || options === void 0
                ? void 0
                : options.autoUpdateHumanBones;
        this.expressionPlugin =
            (_a =
                options === null || options === void 0
                    ? void 0
                    : options.expressionPlugin) !== null && _a !== void 0
                ? _a
                : new VRMExpressionLoaderPlugin(parser);
        this.firstPersonPlugin =
            (_b =
                options === null || options === void 0
                    ? void 0
                    : options.firstPersonPlugin) !== null && _b !== void 0
                ? _b
                : new VRMFirstPersonLoaderPlugin(parser);
        this.humanoidPlugin =
            (_c =
                options === null || options === void 0
                    ? void 0
                    : options.humanoidPlugin) !== null && _c !== void 0
                ? _c
        ...
        this.lookAtPlugin =
            (_d =
                options === null || options === void 0
                    ? void 0
                    : options.lookAtPlugin) !== null && _d !== void 0
                ? _d
                : new VRMLookAtLoaderPlugin(parser, { helperRoot });
        this.metaPlugin =
            (_e =
                options === null || options === void 0
                    ? void 0
                    : options.metaPlugin) !== null && _e !== void 0
                ? _e
                : new VRMMetaLoaderPlugin(parser);
    }
    afterRoot(gltf) {
        return __awaiter$6(this, void 0, void 0, function* () {
...
/*!
 * @pixiv/three-vrm-materials-mtoon v2.0.9
...
class MToonMaterial extends ShaderMaterial {
    ...
    constructor(parameters = {}) {
        ...
        this.uniforms = UniformsUtils.merge([
            ...
            UniformsLib.lights,
            {
                litFactor: { value: new Color(1.0, 1.0, 1.0) },
                ...
                matcapFactor: { value: new Color(0.0, 0.0, 0.0) },
                matcapTexture: { value: null },
                matcapTextureUvTransform: { value: new Matrix3() },
                parametricRimColorFactor: { value: new Color(0.0, 0.0, 0.0) },
                ...
                emissive: { value: new Color(0.0, 0.0, 0.0) },
                ...
                outlineColorFactor: { value: new Color(0.0, 0.0, 0.0) },
        ...
        this.customProgramCacheKey = () =>
            [
                ...Object.entries(this._generateDefines()).map(
                    ([token, macro]) => `${token}:${macro}`
                ),
                this.matcapTexture
                    ? `matcapTextureColorSpace:${getTextureColorSpace(
                          this.matcapTexture
                      )}`
                    : "",
                this.shadeMultiplyTexture
                    ? `shadeMultiplyTextureColorSpace:${getTextureColorSpace(
                          this.shadeMultiplyTexture
                      )}`
                    : "",
                this.rimMultiplyTexture
                    ? `rimMultiplyTextureColorSpace:${getTextureColorSpace(
                          this.rimMultiplyTexture
                      )}`
                    : "",
            ].join(",");
        this.onBeforeCompile = (shader) => {
            ...
            const defines =
                Object.entries(
                    Object.assign(
                        Object.assign({}, this._generateDefines()),
                        this.defines
                    )
                )
            ...
            // -- compat ---------------------------------------------------------------------------------
            // COMPAT: pre-r154
            // js r154 renames the shader chunk <colorspace_fragment> to <encodings_fragment>
    ...
    copy(source) {
        ...
        this.uvAnimationScrollXSpeedFactor =
            source.uvAnimationScrollXSpeedFactor;
        this.uvAnimationScrollYSpeedFactor =
            source.uvAnimationScrollYSpeedFactor;
        this.uvAnimationRotationSpeedFactor =
            source.uvAnimationRotationSpeedFactor;
    ...
    _updateUVAnimation(delta) {
        this.uniforms.uvAnimationScrollXOffset.value +=
            delta * this.uvAnimationScrollXSpeedFactor;
        this.uniforms.uvAnimationScrollYOffset.value +=
            delta * this.uvAnimationScrollYSpeedFactor;
        this.uniforms.uvAnimationRotationPhase.value +=
            delta * this.uvAnimationRotationSpeedFactor;
    ...
    _uploadUniformsWorkaround() {
        ...
        this._updateTextureMatrix(
            this.uniforms.map,
            this.uniforms.mapUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.normalMap,
            this.uniforms.normalMapUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.emissiveMap,
            this.uniforms.emissiveMapUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.shadeMultiplyTexture,
            this.uniforms.shadeMultiplyTextureUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.shadingShiftTexture,
            this.uniforms.shadingShiftTextureUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.matcapTexture,
            this.uniforms.matcapTextureUvTransform
        );
        this._updateTextureMatrix(
            this.uniforms.rimMultiplyTexture,
            this.uniforms.rimMultiplyTextureUvTransform
        );
        this._updateTextureMatrix(
        ...
        this._updateTextureMatrix(
            this.uniforms.uvAnimationMaskTexture,
            this.uniforms.uvAnimationMaskTextureUvTransform
        );
...
class GLTFMToonMaterialParamsAssignHelper {
        ...
        return __awaiter$4(this, void 0, void 0, function* () {
            const promise = (() =>
                __awaiter$4(this, void 0, void 0, function* () {
                    if (texture != null) {
                        yield this._parser.assignTexture(
    ...
    assignTextureByIndex(key, textureIndex, isColorTexture) {
        return __awaiter$4(this, void 0, void 0, function* () {
            return this.assignTexture(
                key,
                textureIndex != null ? { index: textureIndex } : undefined,
                isColorTexture
            );
        });
    }
...
class MToonMaterialLoaderPlugin {
    ...
    constructor(parser, options = {}) {
        var _a, _b, _c;
        this.parser = parser;
        this.renderOrderOffset =
            (_a = options.renderOrderOffset) !== null && _a !== void 0 ? _a : 0;
        this.v0CompatShade =
            (_b = options.v0CompatShade) !== null && _b !== void 0 ? _b : false;
        this.debugMode =
            (_c = options.debugMode) !== null && _c !== void 0 ? _c : "none";
    ...
    afterRoot(gltf) {
        return __awaiter$4(this, void 0, void 0, function* () {
            gltf.userData.vrmMToonMaterials = Array.from(
                this._mToonMaterialSet
            );
        });
    }
    ...
    _removeUnlitExtensionIfMToonExists() {
        ...
        materialDefs === null || materialDefs === void 0
            ? void 0
            : materialDefs.map((materialDef, iMaterial) => {
                  ...
                  if (
                      extension &&
                      ((_a = materialDef.extensions) === null || _a === void 0
                          ? void 0
                          : _a["KHR_materials_unlit"])
                  ) {
    ...
    _getMToonExtension(materialIndex) {
        ...
        const json = parser.json;
        const materialDef =
            (_a = json.materials) === null || _a === void 0
                ? void 0
                : _a[materialIndex];
        if (materialDef == null) {
            console.warn(
        ...
        const extension =
            (_b = materialDef.extensions) === null || _b === void 0
                ? void 0
                : _b[MToonMaterialLoaderPlugin.EXTENSION_NAME];
        if (extension == null) {
            return undefined;
        }
        const specVersion = extension.specVersion;
    ...
    _extendMaterialParams(extension, materialParams) {
        ...
        return __awaiter$4(this, void 0, void 0, function* () {
            ...
            const assignHelper = new GLTFMToonMaterialParamsAssignHelper(
                this.parser,
                materialParams
            );
            assignHelper.assignPrimitive(
                "transparentWithZWrite",
                extension.transparentWithZWrite
            );
            assignHelper.assignColor(
                "shadeColorFactor",
                extension.shadeColorFactor
            );
            assignHelper.assignTexture(
                "shadeMultiplyTexture",
                extension.shadeMultiplyTexture,
                true
            );
            assignHelper.assignPrimitive(
                "shadingShiftFactor",
                extension.shadingShiftFactor
            );
            assignHelper.assignTexture(
                "shadingShiftTexture",
                extension.shadingShiftTexture,
                true
            );
            assignHelper.assignPrimitive(
                "shadingShiftTextureScale",
                (_a = extension.shadingShiftTexture) === null || _a === void 0
                    ? void 0
                    : _a.scale
            );
            assignHelper.assignPrimitive(
                "shadingToonyFactor",
                extension.shadingToonyFactor
            );
            assignHelper.assignPrimitive(
                "giEqualizationFactor",
                extension.giEqualizationFactor
            );
            assignHelper.assignColor("matcapFactor", extension.matcapFactor);
            assignHelper.assignTexture(
                "matcapTexture",
                extension.matcapTexture,
                true
            );
            assignHelper.assignColor(
                "parametricRimColorFactor",
                extension.parametricRimColorFactor
            );
            assignHelper.assignTexture(
                "rimMultiplyTexture",
                extension.rimMultiplyTexture,
                true
            );
            assignHelper.assignPrimitive(
                "rimLightingMixFactor",
                extension.rimLightingMixFactor
            );
            assignHelper.assignPrimitive(
            ...
            assignHelper.assignPrimitive(
                ...
                extension.parametricRimLiftFactor
            );
            assignHelper.assignPrimitive(
                "outlineWidthMode",
                extension.outlineWidthMode
            );
            assignHelper.assignPrimitive(
                "outlineWidthFactor",
                extension.outlineWidthFactor
            );
            assignHelper.assignTexture(
                "outlineWidthMultiplyTexture",
                extension.outlineWidthMultiplyTexture,
                false
            );
            assignHelper.assignColor(
                "outlineColorFactor",
                extension.outlineColorFactor
            );
            assignHelper.assignPrimitive(
                "outlineLightingMixFactor",
                extension.outlineLightingMixFactor
            );
            assignHelper.assignTexture(
                "uvAnimationMaskTexture",
                extension.uvAnimationMaskTexture,
                false
            );
            assignHelper.assignPrimitive(
                "uvAnimationScrollXSpeedFactor",
                extension.uvAnimationScrollXSpeedFactor
            );
            assignHelper.assignPrimitive(
                "uvAnimationScrollYSpeedFactor",
                extension.uvAnimationScrollYSpeedFactor
            );
            assignHelper.assignPrimitive(
                "uvAnimationRotationSpeedFactor",
                extension.uvAnimationRotationSpeedFactor
            );
    ...
    _generateOutline(mesh) {
        ...
        if (
            surfaceMaterial.outlineWidthMode === "none" ||
            surfaceMaterial.outlineWidthFactor <= 0.0
        ) {
            return;
        }
    ...
    _parseRenderOrder(extension) {
        ...
        return (
            (enabledZWrite ? 0 : 19) +
            ((_a = extension.renderQueueOffsetNumber) !== null && _a !== void 0
                ? _a
                : 0)
        );
    }
...
/*!
 * @pixiv/three-vrm-materials-hdr-emissive-multiplier v2.0.9
...
class VRMMaterialsHDREmissiveMultiplierLoaderPlugin {
    ...
    extendMaterialParams(materialIndex, materialParams) {
        return __awaiter$3(this, void 0, void 0, function* () {
            const extension =
                this._getHDREmissiveMultiplierExtension(materialIndex);
            if (extension == null) {
                return;
            }
...
VRMMaterialsHDREmissiveMultiplierLoaderPlugin.EXTENSION_NAME =
    "VRMC_materials_hdr_emissiveMultiplier";

/*!
 * @pixiv/three-vrm-materials-v0compat v2.0.9
...
class VRMMaterialsV0CompatPlugin {
    ...
    constructor(parser) {
        ...
        const json = this.parser.json;
        json.extensionsUsed =
            (_a = json.extensionsUsed) !== null && _a !== void 0 ? _a : [];
    ...
    beforeRoot() {
        ...
        return __awaiter$2(this, void 0, void 0, function* () {
            ...
            const v0VRMExtension =
                (_a = json.extensions) === null || _a === void 0
                    ? void 0
                    : _a["VRM"];
            const v0MaterialProperties =
                v0VRMExtension === null || v0VRMExtension === void 0
                    ? void 0
                    : v0VRMExtension.materialProperties;
            ...
            v0MaterialProperties.forEach(
                (materialProperties, materialIndex) => {
                    var _a, _b;
                    const materialDef =
                        (_a = json.materials) === null || _a === void 0
                            ? void 0
                    ...
                    if (materialProperties.shader === "VRM/MToon") {
                        const material = this._parseV0MToonProperties(
                            materialProperties,
                            materialDef
                        );
                        json.materials[materialIndex] = material;
                    } else if (
                        (_b = materialProperties.shader) === null ||
                        _b === void 0
                            ? void 0
                            : _b.startsWith("VRM/Unlit")
                    ) {
                        const material = this._parseV0UnlitProperties(
                            materialProperties,
                            materialDef
                        );
                        json.materials[materialIndex] = material;
                    } else if (
                        materialProperties.shader === "VRM_USE_GLTFSHADER"
                    );
                    else {
                        console.warn(
                            `VRMMaterialsV0CompatPlugin: Unknown shader: ${materialProperties.shader}`
                        );
                    }
                }
            );
    ...
    _parseV0MToonProperties(materialProperties, schemaMaterial) {
        ...
        const isTransparent =
            (_b =
                (_a = materialProperties.keywordMap) === null || _a === void 0
                    ? void 0
                    : _a["_ALPHABLEND_ON"]) !== null && _b !== void 0
                ? _b
                : false;
        const enabledZWrite =
            ((_c = materialProperties.floatProperties) === null || _c === void 0
                ? void 0
                : _c["_ZWrite"]) === 1;
        ...
        const isCutoff =
            (_e =
                (_d = materialProperties.keywordMap) === null || _d === void 0
                    ? void 0
                    : _d["_ALPHATEST_ON"]) !== null && _e !== void 0
                ? _e
                : false;
        const alphaMode = isTransparent
            ? "BLEND"
            : isCutoff
            ? "MASK"
            : "OPAQUE";
        const alphaCutoff = isCutoff
            ? (_f = materialProperties.floatProperties) === null ||
              _f === void 0
                ? void 0
                : _f["_Cutoff"]
            : undefined;
        const cullMode =
            (_h =
                (_g = materialProperties.floatProperties) === null ||
                _g === void 0
                    ? void 0
                    : _g["_CullMode"]) !== null && _h !== void 0
                ? _h
                : 2; // enum, { Off, Front, Back }
        const doubleSided = cullMode === 0;
        const textureTransformExt =
            this._portTextureTransform(materialProperties);
        const baseColorFactor =
            (_k =
                (_j = materialProperties.vectorProperties) === null ||
                _j === void 0
                    ? void 0
                    : _j["_Color"]) === null || _k === void 0
                ? void 0
                : _k.map((v, i) => (i === 3 ? v : gammaEOTF(v)));
        const baseColorTextureIndex =
            (_l = materialProperties.textureProperties) === null ||
            _l === void 0
                ? void 0
                : _l["_MainTex"];
        ...
        const normalTextureScale =
            (_m = materialProperties.floatProperties) === null || _m === void 0
                ? void 0
                : _m["_BumpScale"];
        const normalTextureIndex =
            (_o = materialProperties.textureProperties) === null ||
            _o === void 0
                ? void 0
                : _o["_BumpMap"];
        const normalTexture =
        ...
        const emissiveFactor =
            (_q =
                (_p = materialProperties.vectorProperties) === null ||
                _p === void 0
                    ? void 0
                    : _p["_EmissionColor"]) === null || _q === void 0
                ? void 0
                : _q.map(gammaEOTF);
        const emissiveTextureIndex =
            (_r = materialProperties.textureProperties) === null ||
            _r === void 0
                ? void 0
                : _r["_EmissionMap"];
        ...
        const shadeColorFactor =
            (_t =
                (_s = materialProperties.vectorProperties) === null ||
                _s === void 0
                    ? void 0
                    : _s["_ShadeColor"]) === null || _t === void 0
                ? void 0
                : _t.map(gammaEOTF);
        const shadeMultiplyTextureIndex =
            (_u = materialProperties.textureProperties) === null ||
            _u === void 0
                ? void 0
                : _u["_ShadeTexture"];
        ...
        let shadingShiftFactor =
            (_w =
                (_v = materialProperties.floatProperties) === null ||
                _v === void 0
                    ? void 0
                    : _v["_ShadeShift"]) !== null && _w !== void 0
                ? _w
                : 0.0;
        let shadingToonyFactor =
            (_y =
                (_x = materialProperties.floatProperties) === null ||
                _x === void 0
                    ? void 0
                    : _x["_ShadeToony"]) !== null && _y !== void 0
                ? _y
                : 0.9;
        shadingToonyFactor = MathUtils.lerp(
            shadingToonyFactor,
            1.0,
        ...
        const giIntensityFactor =
            (_z = materialProperties.floatProperties) === null || _z === void 0
                ? void 0
                : _z["_IndirectLightIntensity"];
        const giEqualizationFactor = giIntensityFactor
            ? 1.0 - giIntensityFactor
            : undefined;
        const matcapTextureIndex =
            (_0 = materialProperties.textureProperties) === null ||
            _0 === void 0
                ? void 0
                : _0["_SphereAdd"];
        const matcapFactor =
            matcapTextureIndex != null ? [1.0, 1.0, 1.0] : undefined;
        const matcapTexture =
        ...
        const rimLightingMixFactor =
            (_1 = materialProperties.floatProperties) === null || _1 === void 0
                ? void 0
                : _1["_RimLightingMix"];
        const rimMultiplyTextureIndex =
            (_2 = materialProperties.textureProperties) === null ||
            _2 === void 0
                ? void 0
                : _2["_RimTexture"];
        ...
        const parametricRimColorFactor =
            (_4 =
                (_3 = materialProperties.vectorProperties) === null ||
                _3 === void 0
                    ? void 0
                    : _3["_RimColor"]) === null || _4 === void 0
                ? void 0
                : _4.map(gammaEOTF);
        const parametricRimFresnelPowerFactor =
            (_5 = materialProperties.floatProperties) === null || _5 === void 0
                ? void 0
                : _5["_RimFresnelPower"];
        const parametricRimLiftFactor =
            (_6 = materialProperties.floatProperties) === null || _6 === void 0
                ? void 0
                : _6["_RimLift"];
        const outlineWidthMode = [
            ...
            (_8 =
                (_7 = materialProperties.floatProperties) === null ||
                _7 === void 0
                    ? void 0
                    : _7["_OutlineWidthMode"]) !== null && _8 !== void 0
                ? _8
                : 0
        ];
        ...
        let outlineWidthFactor =
            (_10 =
                (_9 = materialProperties.floatProperties) === null ||
                _9 === void 0
                    ? void 0
                    : _9["_OutlineWidth"]) !== null && _10 !== void 0
                ? _10
                : 0.0;
        outlineWidthFactor = 0.01 * outlineWidthFactor;
        const outlineWidthMultiplyTextureIndex =
            (_11 = materialProperties.textureProperties) === null ||
            _11 === void 0
                ? void 0
                : _11["_OutlineWidthTexture"];
        ...
        const outlineColorFactor =
            (_13 =
                (_12 = materialProperties.vectorProperties) === null ||
                _12 === void 0
                    ? void 0
                    : _12["_OutlineColor"]) === null || _13 === void 0
                ? void 0
                : _13.map(gammaEOTF);
        const outlineColorMode =
            (_14 = materialProperties.floatProperties) === null ||
            _14 === void 0
                ? void 0
                : _14["_OutlineColorMode"]; // enum, { Fixed, Mixed }
        const outlineLightingMixFactor =
            outlineColorMode === 1
                ? (_15 = materialProperties.floatProperties) === null ||
                  _15 === void 0
                    ? void 0
                    : _15["_OutlineLightingMix"]
                : 0.0;
        const uvAnimationMaskTextureIndex =
            (_16 = materialProperties.textureProperties) === null ||
            _16 === void 0
                ? void 0
                : _16["_UvAnimMaskTexture"];
        ...
        const uvAnimationRotationSpeedFactor =
            (_19 = materialProperties.floatProperties) === null ||
            _19 === void 0
                ? void 0
                : _19["_UvAnimRotation"];
    ...
    _parseV0UnlitProperties(materialProperties, schemaMaterial) {
        var _a, _b, _c, _d;
        const isTransparentZWrite =
            materialProperties.shader === "VRM/UnlitTransparentZWrite";
        const isTransparent =
        ...
        const alphaMode = isTransparent
            ...
            : "OPAQUE";
        const alphaCutoff = isCutoff
            ? (_a = materialProperties.floatProperties) === null ||
              _a === void 0
                ? void 0
                : _a["_Cutoff"]
            : undefined;
        ...
        const baseColorFactor =
            (_c =
                (_b = materialProperties.vectorProperties) === null ||
                _b === void 0
                    ? void 0
                    : _b["_Color"]) === null || _c === void 0
                ? void 0
                : _c.map(gammaEOTF);
        const baseColorTextureIndex =
            (_d = materialProperties.textureProperties) === null ||
            _d === void 0
                ? void 0
                : _d["_MainTex"];
        const baseColorTexture =
    ...
    _portTextureTransform(materialProperties) {
        ...
        const textureTransform =
            (_a = materialProperties.vectorProperties) === null || _a === void 0
                ? void 0
                : _a["_MainTex"];
        ...
        const offset = [
            (_b =
                textureTransform === null || textureTransform === void 0
                    ? void 0
                    : textureTransform[0]) !== null && _b !== void 0
                ? _b
                : 0.0,
            (_c =
                textureTransform === null || textureTransform === void 0
                    ? void 0
                    : textureTransform[1]) !== null && _c !== void 0
                ? _c
                : 0.0,
        ];
        const scale = [
            (_d =
                textureTransform === null || textureTransform === void 0
                    ? void 0
                    : textureTransform[2]) !== null && _d !== void 0
                ? _d
                : 1.0,
            (_e =
                textureTransform === null || textureTransform === void 0
                    ? void 0
                    : textureTransform[3]) !== null && _e !== void 0
                ? _e
                : 1.0,
        ];
    ...
    _populateRenderQueueMap(materialPropertiesList) {
        ...
        materialPropertiesList.forEach((materialProperties) => {
            ...
            const isTransparent =
                (_b =
                    (_a = materialProperties.keywordMap) === null ||
                    _a === void 0
                        ? void 0
                        : _a["_ALPHABLEND_ON"]) !== null && _b !== void 0
                    ? _b
                    : false;
            const enabledZWrite =
                ((_c = materialProperties.floatProperties) === null ||
                _c === void 0
                    ? void 0
                    : _c["_ZWrite"]) === 1;
            if (isTransparent) {
        ...
        Array.from(renderQueuesTransparent)
            .sort()
            .forEach((queue, i) => {
                const newQueueOffset = Math.min(
                    Math.max(i - renderQueuesTransparent.size + 1, -9),
                    0
                );
        ...
        Array.from(renderQueuesTransparentZWrite)
            ...
            .forEach((queue, i) => {
                ...
                this._renderQueueMapTransparentZWrite.set(
                    queue,
                    newQueueOffset
                );
            });
...
/*!
 * @pixiv/three-vrm-node-constraint v2.0.9
...
const _v3A$3$1 = new Vector3();
class VRMNodeConstraintHelper extends Group {
    constructor(constraint) {
        super();
        this._attrPosition = new BufferAttribute(
...
const _v3A$2$1 = new Vector3();
const _v3B$1$1 = new Vector3();
...
const _v3A$1$1 = new Vector3();
const _v3B$2 = new Vector3();
const _v3C$1 = new Vector3();
const _quatA$2 = new Quaternion();
const _quatB$2 = new Quaternion();
const _quatC = new Quaternion();
/**
...
class VRMAimConstraint extends VRMNodeConstraint {
    ...
    update() {
        ...
        if (this.destination.parent) {
            decomposeRotation(
                this.destination.parent.matrixWorld,
                dstParentWorldQuat
            );
        ...
        const a0 = _v3A$1$1
            .copy(this._v3AimAxis)
            .applyQuaternion(this._dstRestQuat)
            .applyQuaternion(dstParentWorldQuat);
        ...
        this.destination.quaternion
            .copy(this._dstRestQuat)
            .slerp(targetQuat, this.weight);
    }
}
...
class VRMNodeConstraintManager {
    ...
    deleteConstraint(constraint) {
        ...
        const objectSet = this._objectConstraintsMap.get(
    ...
    setInitState() {
        ...
        for (const constraint of this._constraints) {
            this._processConstraint(
                constraint,
                constraintsTried,
                constraintsDone,
                (constraint) => constraint.setInitState()
            );
        }
    }
    update() {
    ...
    _processConstraint(
        ...
        if (constraintsTried.has(constraint)) {
            throw new Error(
                "VRMNodeConstraintManager: Circular dependency detected while updating constraints"
            );
        }
        ...
        for (const depObject of depObjects) {
            traverseAncestorsFromRoot$1(depObject, (depObjectAncestor) => {
                const objectSet =
                    this._objectConstraintsMap.get(depObjectAncestor);
                if (objectSet) {
...
const _v3A$5 = new Vector3();
const _quatA$3 = new Quaternion();
...
class VRMRollConstraint extends VRMNodeConstraint {
    ...
    update() {
        ...
        const targetQuat = quatFromTo
            .premultiply(this._dstRestQuat)
            .multiply(quatDelta);
        // blend with the rest quaternion using weight
        this.destination.quaternion
            .copy(this._dstRestQuat)
        ...
        return __awaiter$1(this, void 0, void 0, function* () {
            ...
            const isConstraintsUsed =
                ((_a = json.extensionsUsed) === null || _a === void 0
                    ? void 0
                    : _a.indexOf(
                          VRMNodeConstraintLoaderPlugin.EXTENSION_NAME
                      )) !== -1;
            ...
            threeNodes.forEach((node, nodeIndex) => {
                ...
                const extension =
                    (_a =
                        schemaNode === null || schemaNode === void 0
                            ? void 0
                            : schemaNode.extensions) === null || _a === void 0
                        ? void 0
                        : _a[VRMNodeConstraintLoaderPlugin.EXTENSION_NAME];
                if (extension == null) {
                    return;
                }
                const specVersion = extension.specVersion;
                ...
                if (constraintDef.roll != null) {
                    const constraint = this._importRollConstraint(
                        node,
                        threeNodes,
                        constraintDef.roll
                    );
                    ...
                    const constraint = this._importAimConstraint(
                        node,
                        threeNodes,
                        constraintDef.aim
                    );
                    ...
                    const constraint = this._importRotationConstraint(
                        node,
                        threeNodes,
                        constraintDef.rotation
                    );
                    manager.addConstraint(constraint);
                }
            });
        ...
        this.offset =
            (_a =
                params === null || params === void 0
                    ? void 0
                    : params.offset) !== null && _a !== void 0
                ? _a
                : new Vector3(0.0, 0.0, 0.0);
        this.tail =
            (_b =
                params === null || params === void 0 ? void 0 : params.tail) !==
                null && _b !== void 0
                ? _b
                : new Vector3(0.0, 0.0, 0.0);
        this.radius =
...
class ColliderShapeCapsuleBufferGeometry extends BufferGeometry {
    constructor(shape) {
        super();
        this.worldScale = 1.0;
        this._currentRadius = 0;
    ...
    _buildPosition() {
        ...
        const theta = Math.atan2(
            _v3A$3.y,
            Math.sqrt(_v3A$3.x * _v3A$3.x + _v3A$3.z * _v3A$3.z)
        );
...
class ColliderShapeSphereBufferGeometry extends BufferGeometry {
    constructor(shape) {
        super();
        this.worldScale = 1.0;
        this._currentRadius = 0;
...
class VRMSpringBoneColliderHelper extends Group {
    constructor(collider) {
        super();
        this.matrixAutoUpdate = false;
        this.collider = collider;
        if (this.collider.shape instanceof VRMSpringBoneColliderShapeSphere) {
            this._geometry = new ColliderShapeSphereBufferGeometry(
                this.collider.shape
            );
        } else if (
            this.collider.shape instanceof VRMSpringBoneColliderShapeCapsule
        ) {
            this._geometry = new ColliderShapeCapsuleBufferGeometry(
                this.collider.shape
            );
        } else {
            throw new Error(
                "VRMSpringBoneColliderHelper: Unknown collider shape type detected"
            );
        }
        const material = new LineBasicMaterial({
            color: 0xff00ff,
    ...
    updateMatrixWorld(force) {
        ...
        this._geometry.worldScale = _v3A$2
            .set(
                matrixWorldElements[0],
                matrixWorldElements[1],
                matrixWorldElements[2]
            )
...
class SpringBoneBufferGeometry extends BufferGeometry {
    ...
    update() {
        ...
        if (
            !this._currentTail.equals(
                this._springBone.initialLocalChildPosition
            )
        ) {
...
class VRMSpringBoneJointHelper extends Group {
    constructor(springBone) {
        super();
        this.matrixAutoUpdate = false;
 ...
 */
class VRMSpringBoneCollider extends Object3D {
    constructor(shape) {
        super();
        this.shape = shape;
    }
...
const IDENTITY_MATRIX4 = new Matrix4();
 ...
 */
const _worldSpacePosition = new Vector3();
/**
 * A temporary variable which is used in `update`
 */
const _centerSpacePosition = new Vector3();
/**
 * A temporary variable which is used in `update`
 */
const _nextTail = new Vector3();
...
class VRMSpringBoneJoint {
        ...
        if (this._center) {
            if (!this._center.userData.inverseCacheProxy) {
                this._center.userData.inverseCacheProxy =
                    new Matrix4InverseCache(this._center.matrixWorld);
            }
        }
    }
    ...
    constructor(bone, child, settings = {}, colliderGroups = []) {
        ...
        this.settings = {
            hitRadius:
                (_a = settings.hitRadius) !== null && _a !== void 0 ? _a : 0.0,
            stiffness:
                (_b = settings.stiffness) !== null && _b !== void 0 ? _b : 1.0,
            gravityPower:
                (_c = settings.gravityPower) !== null && _c !== void 0
                    ? _c
                    : 0.0,
            gravityDir:
                (_e =
                    (_d = settings.gravityDir) === null || _d === void 0
                        ? void 0
                        : _d.clone()) !== null && _e !== void 0
                    ? _e
                    : new Vector3(0.0, -1.0, 0.0),
            dragForce:
                (_f = settings.dragForce) !== null && _f !== void 0 ? _f : 0.4,
        };
    ...
    setInitState() {
        ...
        this.bone
            ...
            .applyMatrix4(matrixWorldToCenter);
    ...
    update(delta) {
        ...
        _centerSpacePosition
            .copy(_worldSpacePosition)
            .applyMatrix4(matrixWorldToCenter);
        const quatWorldToCenter =
            _quatA.setFromRotationMatrix(matrixWorldToCenter);
        // Get parent matrix in center space
        const centerSpaceParentMatrix = _matB
            .copy(matrixWorldToCenter)
            .multiply(this._parentMatrixWorld);
        ...
        const centerSpaceGravity = _v3C
            .copy(this.settings.gravityDir)
            .applyQuaternion(quatWorldToCenter)
            .normalize();
        ...
        _nextTail
            ...
            .add(
                _v3A
                    .copy(centerSpaceBoneAxis)
                    .multiplyScalar(this.settings.stiffness * delta)
            ) // 親の回転による子ボーンの移動目標
            .add(
                _v3A
                    .copy(centerSpaceGravity)
        ...
        this._currentTail.copy(
            _v3A.copy(_nextTail).applyMatrix4(matrixWorldToCenter)
        );
        ...
        const worldSpaceInitialMatrixInv = mat4InvertCompat(
            _matA
                ...
                .multiply(this._initialLocalMatrix)
        );
        const applyRotation = _quatA.setFromUnitVectors(
            this._boneAxis,
            _v3A
                .copy(_nextTail)
                .applyMatrix4(worldSpaceInitialMatrixInv)
                .normalize()
        );
...
class VRMSpringBoneManager {
    ...
    get springBones() {
        console.warn(
            "VRMSpringBoneManager: springBones is deprecated. use joints instead."
        );
        return this._joints;
    }
    get colliderGroups() {
    ...
    addSpringBone(joint) {
        console.warn(
            "VRMSpringBoneManager: addSpringBone() is deprecated. use addJoint() instead."
        );
        this.addJoint(joint);
    }
    deleteJoint(joint) {
    ...
    deleteSpringBone(joint) {
        console.warn(
            "VRMSpringBoneManager: deleteSpringBone() is deprecated. use deleteJoint() instead."
        );
    ...
    setInitState() {
        ...
        for (const springBone of this._joints) {
            this._processSpringBone(
                ...
                springBonesDone,
                objectUpdated,
                (springBone) => springBone.setInitState()
            );
        }
    }
    reset() {
    ...
    update(delta) {
        ...
        for (const springBone of this._joints) {
            ...
            this._processSpringBone(
                ...
                springBonesDone,
                objectUpdated,
                (springBone) => springBone.update(delta)
            );
            ...
            traverseChildrenUntilConditionMet(springBone.bone, (object) => {
                ...
                if (
                    ((_b =
                        (_a = this._objectSpringBonesMap.get(object)) ===
                            null || _a === void 0
                            ? void 0
    ...
    _processSpringBone(
        ...
        for (const depObject of depObjects) {
            traverseAncestorsFromRoot(depObject, (depObjectAncestor) => {
                const objectSet =
                    this._objectSpringBonesMap.get(depObjectAncestor);
                if (objectSet) {
...
class VRMSpringBoneLoaderPlugin {
    ...
    constructor(parser, options) {
        this.parser = parser;
        this.jointHelperRoot =
            options === null || options === void 0
                ? void 0
                : options.jointHelperRoot;
        this.colliderHelperRoot =
            options === null || options === void 0
                ? void 0
                : options.colliderHelperRoot;
    }
    afterRoot(gltf) {
    ...
    _v1Import(gltf) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const isSpringBoneUsed =
                ((_a = json.extensionsUsed) === null || _a === void 0
                    ? void 0
                    : _a.indexOf(VRMSpringBoneLoaderPlugin.EXTENSION_NAME)) !==
                -1;
            if (!isSpringBoneUsed) {
                return null;
            }
            ...
            const extension =
                (_b = json.extensions) === null || _b === void 0
                    ? void 0
                    : _b[VRMSpringBoneLoaderPlugin.EXTENSION_NAME];
            if (!extension) {
                return null;
            }
            const specVersion = extension.specVersion;
            ...
            const colliders =
                (_c = extension.colliders) === null || _c === void 0
                    ? void 0
                    : _c.map((schemaCollider, iCollider) => {
                          var _a, _b, _c, _d, _e;
                          const node = threeNodes[schemaCollider.node];
                          const schemaShape = schemaCollider.shape;
                          if (schemaShape.sphere) {
                              return this._importSphereCollider(node, {
                                  offset: new Vector3().fromArray(
                                      (_a = schemaShape.sphere.offset) !==
                                          null && _a !== void 0
                                          ? _a
                                          : [0.0, 0.0, 0.0]
                                  ),
                                  radius:
                                      (_b = schemaShape.sphere.radius) !==
                                          null && _b !== void 0
                                          ? _b
                                          : 0.0,
                              });
                              ...
                              return this._importCapsuleCollider(node, {
                                  offset: new Vector3().fromArray(
                                      (_c = schemaShape.capsule.offset) !==
                                          null && _c !== void 0
                                          ? _c
                                          : [0.0, 0.0, 0.0]
                                  ),
                                  radius:
                                      (_d = schemaShape.capsule.radius) !==
                                          null && _d !== void 0
                                          ? _d
                                          : 0.0,
                                  tail: new Vector3().fromArray(
                                      (_e = schemaShape.capsule.tail) !==
                                          null && _e !== void 0
                                          ? _e
                                          : [0.0, 0.0, 0.0]
                                  ),
                          ...
                          throw new Error(
                              `VRMSpringBoneLoaderPlugin: The collider #${iCollider} has no valid shape`
                          );
                      });
            const colliderGroups =
                (_d = extension.colliderGroups) === null || _d === void 0
                    ? void 0
                    : _d.map((schemaColliderGroup, iColliderGroup) => {
                          var _a;
                          const cols = (
                              (_a = schemaColliderGroup.colliders) !== null &&
                              _a !== void 0
                                  ? _a
                                  : []
                          ).map((iCollider) => {
                              const col =
                                  colliders === null || colliders === void 0
                                      ? void 0
                                      : colliders[iCollider];
                              if (col == null) {
                                  throw new Error(
            ...
            (_e = extension.springs) === null || _e === void 0
                ? void 0
                : _e.forEach((schemaSpring, iSpring) => {
                      var _a;
                      const schemaJoints = schemaSpring.joints;
                      // prepare colliders
                      const colliderGroupsForSpring =
                          (_a = schemaSpring.colliderGroups) === null ||
                          _a === void 0
                              ? void 0
                              : _a.map((iColliderGroup) => {
                                    const group =
                                        colliderGroups === null ||
                                        colliderGroups === void 0
                                            ? void 0
                                            : colliderGroups[iColliderGroup];
                                    if (group == null) {
                                        throw new Error(
                      ...
                      const center =
                          schemaSpring.center != null
                              ? threeNodes[schemaSpring.center]
                              : undefined;
                      let prevSchemaJoint;
                                  ...
                                  gravityDir:
                                      prevSchemaJoint.gravityDir != null
                                          ? new Vector3().fromArray(
                                                prevSchemaJoint.gravityDir
                                            )
                                          : undefined,
                              };
                              // create spring bones
                              const joint = this._importJoint(
                                  node,
                                  child,
                                  setting,
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const extension =
                (_b = json.extensions) === null || _b === void 0
                    ? void 0
                    : _b["VRM"];
            const schemaSecondaryAnimation =
            ...
            const schemaBoneGroups =
                schemaSecondaryAnimation === null ||
                schemaSecondaryAnimation === void 0
                    ? void 0
                    : schemaSecondaryAnimation.boneGroups;
            if (!schemaBoneGroups) {
                return null;
            }
            const manager = new VRMSpringBoneManager();
            const threeNodes = yield gltf.parser.getDependencies("node");
            const colliderGroups =
                (_c = schemaSecondaryAnimation.colliderGroups) === null ||
                _c === void 0
                    ? void 0
                    : _c.map((schemaColliderGroup) => {
                          var _a;
                          ...
                          const colliders = (
                              ...
                              _a !== void 0
                                  ? _a
                                  : []
                          ).map((schemaCollider, iCollider) => {
                              var _a, _b, _c;
                              const offset = new Vector3(0.0, 0.0, 0.0);
                              if (schemaCollider.offset) {
                                  offset.set(
                                      (_a = schemaCollider.offset.x) !== null &&
                                          _a !== void 0
                                          ? _a
                                          : 0.0,
                                      (_b = schemaCollider.offset.y) !== null &&
                                          _b !== void 0
                                          ? _b
                                          : 0.0,
                                      schemaCollider.offset.z
                                          ? -schemaCollider.offset.z
                                          : 0.0
                                  );
                              }
            ...
            schemaBoneGroups === null || schemaBoneGroups === void 0
                ? void 0
                : schemaBoneGroups.forEach((schemaBoneGroup, iBoneGroup) => {
                      ...
                      rootIndices.forEach((rootIndex) => {
                          ...
                          if (schemaBoneGroup.gravityDir) {
                              gravityDir.set(
                                  (_a = schemaBoneGroup.gravityDir.x) !==
                                      null && _a !== void 0
                                      ? _a
                                      : 0.0,
                                  (_b = schemaBoneGroup.gravityDir.y) !==
                                      null && _b !== void 0
                                      ? _b
                                      : 0.0,
                                  (_c = schemaBoneGroup.gravityDir.z) !==
                                      null && _c !== void 0
                                      ? _c
                                      : 0.0
                              );
                          ...
                          const center =
                              schemaBoneGroup.center != null
                                  ? threeNodes[schemaBoneGroup.center]
                                  : undefined;
                          const setting = {
                          ...
                          // prepare colliders
                          const colliderGroupsForSpring =
                              (_d = schemaBoneGroup.colliderGroups) === null ||
                              _d === void 0
                                  ? void 0
                                  : _d.map((iColliderGroup) => {
                                        const group =
                                            colliderGroups === null ||
                                            colliderGroups === void 0
                                                ? void 0
...
class VRMLoaderPlugin {
    ...
    constructor(parser, options) {
        ...
        this.metaPlugin =
            (_e =
                ...
                ? _e
                : new VRMMetaLoaderPlugin(parser);
        this.mtoonMaterialPlugin =
            (_f =
                options === null || options === void 0
                    ? void 0
                    : options.mtoonMaterialPlugin) !== null && _f !== void 0
                ? _f
                : new MToonMaterialLoaderPlugin(parser);
        this.materialsHDREmissiveMultiplierPlugin =
            (_g =
                options === null || options === void 0
                    ? void 0
                    : options.materialsHDREmissiveMultiplierPlugin) !== null &&
            _g !== void 0
                ? _g
                : new VRMMaterialsHDREmissiveMultiplierLoaderPlugin(parser);
        this.materialsV0CompatPlugin =
            (_h =
                options === null || options === void 0
                    ? void 0
                    : options.materialsV0CompatPlugin) !== null && _h !== void 0
                ? _h
                : new VRMMaterialsV0CompatPlugin(parser);
        this.springBonePlugin =
            (_j =
                options === null || options === void 0
                    ? void 0
                    : options.springBonePlugin) !== null && _j !== void 0
                ? _j
        ...
        this.nodeConstraintPlugin =
            (_k =
                options === null || options === void 0
                    ? void 0
                    : options.nodeConstraintPlugin) !== null && _k !== void 0
                ? _k
    ...
    getMaterialType(materialIndex) {
        const mtoonType =
            this.mtoonMaterialPlugin.getMaterialType(materialIndex);
        if (mtoonType != null) {
            return mtoonType;
        }
        return null;
    ...
    extendMaterialParams(materialIndex, materialParams) {
        return __awaiter$5(this, void 0, void 0, function* () {
            yield this.materialsHDREmissiveMultiplierPlugin.extendMaterialParams(
                materialIndex,
                materialParams
            );
            yield this.mtoonMaterialPlugin.extendMaterialParams(
                materialIndex,
                materialParams
            );
        });
            ...
            if (meta && humanoid) {
                const vrm = new VRM({
                    ...
                    nodeConstraintManager:
                        gltf.userData.vrmNodeConstraintManager,
                });
...
function removeUnnecessaryVertices(root) {
    ...
    root.traverse((obj) => {
        ...
        newGeometry.boundingBox =
            (_b =
                (_a = geometry.boundingBox) === null || _a === void 0
                    ? void 0
                    : _a.clone()) !== null && _b !== void 0
                ? _b
                : null;
        newGeometry.boundingSphere =
            (_d =
                (_c = geometry.boundingSphere) === null || _c === void 0
                    ? void 0
                    : _c.clone()) !== null && _d !== void 0
                ? _d
                : null;
        newGeometry.setDrawRange(
        ...
        // reorganize indices
        {
            ...
            const newIndexArray = new originalIndexArray.constructor(
                originalIndexArray.length
            );
            let indexHead = 0;
        ...
        Object.keys(geometry.attributes).forEach((attributeName) => {
            ...
            if (originalAttribute.isInterleavedBufferAttribute) {
                throw new Error(
                    "removeUnnecessaryVertices: InterleavedBufferAttribute is not supported"
                );
            }
            ...
            newIndexOriginalIndexMap.forEach((originalIndex, i) => {
                for (let j = 0; j < itemSize; j++) {
                    newAttributeArray[i * itemSize + j] =
                        originalAttributeArray[originalIndex * itemSize + j];
                }
            });
            newGeometry.setAttribute(
                attributeName,
                new BufferAttribute(newAttributeArray, itemSize, normalized)
            );
        });
        ...
        Object.keys(geometry.morphAttributes).forEach((attributeName) => {
            ...
            for (let iMorph = 0; iMorph < morphs.length; iMorph++) {
                ...
                const newAttributeArray =
                    new originalAttributeArray.constructor(
                        newIndexOriginalIndexMap.length * itemSize
                    );
                newIndexOriginalIndexMap.forEach((originalIndex, i) => {
                    for (let j = 0; j < itemSize; j++) {
                        newAttributeArray[i * itemSize + j] =
                            originalAttributeArray[
                                originalIndex * itemSize + j
                            ];
                    }
                });
                isNullMorph =
                    isNullMorph && newAttributeArray.every((v) => v === 0);
                newGeometry.morphAttributes[attributeName][iMorph] =
                    new BufferAttribute(
                        newAttributeArray,
                        itemSize,
                        normalized
