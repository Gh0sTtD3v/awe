...
class GLTF {
    ...
    loadObject(object) {
        return new Promise((resolve, reject) => {
            ...
            let force = object.force || url.startsWith("blob:");
            ...
            if (this[object.name] != null) {
                ...
                // &&(ISGLB == true || ISVRM == true || object.force == true) // not all glb or vrm links have the extension in their links
            ) {
    ...
    async load(object) {
        // try {
        // try {
        var res = await Subsystems.gltf.loadGLTF(object.url);
    ...
    async getSingleInstance(url) {
        ...
        if (this["raw" + url] != null && this["raw" + url].rawBuffer != null) {
            res = await Subsystems.gltf.parseGLTF(this["raw" + url].rawBuffer);
        } else {
            if (this.rawLoadings[url] == null) {
                this.rawLoadings[url] = new Promise(async (resolve, reject) => {
                    res = await Subsystems.gltf.loadGLTF(url);
