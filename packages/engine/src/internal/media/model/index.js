...
import InstancedModelWrapper from "./instanced-wrapper";

import ClassicWrapper from "./classic-wrapper";

import { disposeThreeResources } from "../../utils/dispose";

import { COMPRESSED_SUPPORT, IS_DESKTOP } from "../../constants";
...
export class ModelFactory {
  ...
  async get(parent, data) {
    ...
    } else if (data.optimized?.low && !IS_DESKTOP) {
    ...
    if (
      ...
      if (
        ...
        let isAnimated = res.animations.length > 0; //&& Object.keys(data.animations ?? {}).length > 0;
        ...
        if (isAnimated == true || data.forceUnique) {
    ...
    if (this.instances[namePlugin] != null) {
      ...
      const result = this.instances[namePlugin].addItem(data);
      result.__cacheKey = namePlugin;
      return result;
    } else {
      ...
      classicWrapper.__cacheKey = namePlugin;
  ...
  removeInstance(wrapper) {
    const namePlugin = wrapper?.__cacheKey;

    if (!namePlugin) return;
      ...
      while (i < opts.plugins.length) {
        ...
        if (opts.plugins[i].uniforms != null) {
          for (var key in opts.plugins[i].uniforms) {
            ...
            if (u != null && typeof u.value === "number") {
              str += key + ":" + u.value + ",";
            }
          }
        }
