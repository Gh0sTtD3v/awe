...
let isGlobalFogPatchInstalled = false;

function applyFogUniforms(shader, material) {
    if (material?.fog === false || shader?.uniforms == null) {
        return;
    }
...
function installGlobalFogUniformPatch() {
    if (isGlobalFogPatchInstalled) {
        return;
    }

    isGlobalFogPatchInstalled = true;
...
let setOnBeforeCompile = (material, name = null ) => {
    ...
    material.onBeforeCompile = (shader) => {
        ...
        applyFogUniforms(shader, material);

        // Add shadowOnlyLightIndex for shadow-only light support
        if (material.receiveShadow !== false) {
            material._data.uniforms.shadowOnlyLightIndex = Shared.shadowOnlyLightIndex;
        }
...
// override
import FogParsFragment from "./override/fog_pars.glsl.ts";
import FogFragment from "./override/midfog.glsl.ts";
import FogParsVertex from "./override/fog-pars-vertex.glsl.ts";
import FogVertex from "./override/fog-vertex.glsl.ts";
import EncodingFragment from "./override/encoding.glsl.ts";

import ShadowParsFragment from './override/shadowparsfrag.glsl.ts'

import LightFragment from './override/lightfragment.glsl.ts'
