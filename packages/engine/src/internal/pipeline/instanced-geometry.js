...
export default class GeometryInstancer extends InstancedBufferGeometry {
    ...
    sort(wrappers, camera = null, forceView = false) {
        ...
        let frustum =
            camera && camera.frustum ? camera.frustum : Camera._frustum;
        ...
        while (x < wrappers.length) {
            ...
            if (
                ...
                // Dynamic-shadow instances are already routed to the correct
                // shadow light via mesh layers, so they still need to stay in
        ...
        while (h < l) {
            ...
            if (scope._maxInstanceCount > 0) {
                const offsetCount = scope._maxInstanceCount * scope.attributes.offset.itemSize;
                scope.attributes.offset.clearUpdateRanges();
                scope.attributes.offset.addUpdateRange(0, offsetCount);
                scope.attributes.offset.needsUpdate = true;

                for (const item in this.items) {
                    const itemCount = scope._maxInstanceCount * scope.attributes[item].itemSize;
                    scope.attributes[item].clearUpdateRanges();
                    scope.attributes[item].addUpdateRange(0, itemCount);
    ...
    rangeNonSorted(instances) {
        ...
        if (this._maxInstanceCount > 0) {
            const offsetCount = this._maxInstanceCount * this.attributes.offset.itemSize;
            ...
            for (const item in this.items) {
                const itemCount = this._maxInstanceCount * this.attributes[item].itemSize;
    ...
    updateAttribute(scope, wrapper, c) {
        ...
        if (this.vrmScale == true) {
            ...
            scope.attributes.scale.array[base3] =
                wrapper.scale.x * this.scaleRatio;
        ...
        if (this.opts.animations) {
            ...
            scope.attributes.animations.array[base4 + 1] =
                wrapper.animations[1];
            scope.attributes.animations.array[base4 + 2] =
                wrapper.animations[2];
            scope.attributes.animations.array[base4 + 3] =
                wrapper.animations[3];
        }
            ...
            if (scope.copyBuffer != null) {
                ...
                for (const item in scope.items) {
                    ...
                    scope.setAttribute(
                        ...
                        new InstancedBufferAttribute(
                            new type(
                                new Array(scope.items[item].length * this.max)
                            ),
    ...
    /**
     * Returns the array of LOD geometries.
     * Used by LODInstancedGroup to create separate meshes per LOD level.
     * @returns {Array} Array of InstancedBufferGeometry objects, one per LOD level
     */
    getLODGeometries() {
        return this._lods || [this];
    }
}
