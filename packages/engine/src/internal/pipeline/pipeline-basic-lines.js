...
import EventEmitter from "../events/augmented";
...
export default class PipeLineBasicLine extends LineSegments {
  constructor(geometry, material, rule = {}) {
    ...
    this.emitter = new EventEmitter({ scope: "engine" });
  ...
  // @ts-ignore - intentional override of parent material setter
  set material(val) {
    this._material = val;
  ...
  _onOcclusion(val) {
    ...
    if (
      // @ts-ignore - internal geometry property
  ...
  _addEvents() {
    ...
    if (this.useLighting && this.lightingEvent == null) {
      ...
      emitter.on(EngineEvents.LIGHTING, this.lightingEvent);
    }
  }
  ...
  _removeEvents() {
    ...
    if (this.lightingEvent) {
      emitter.off(EngineEvents.LIGHTING, this.lightingEvent);
    }
  }
  destroy() {
