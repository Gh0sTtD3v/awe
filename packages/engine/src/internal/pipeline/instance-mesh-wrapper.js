...
export default class InstancedMeshWrapper {
    ...
    attachTo(source, callback = null) {
        this.__objectSource = source;
        this.__objectCallback = callback;
    }
    ...
    updateFromSource(source = null) {
        ...
        if (this.__objectCallback) {
            this.__objectCallback();
        }

        this._updateCollisionMesh();
    }
    ...
    buildCollisionMesh() {
        ...
        const mesh = new InstancedWrapperCollisionMesh(
            geometry,
            material,
            this
        );
    ...
    getCollisionMesh() {
        ...
        if (this._collisionMesh == null) {
            this._collisionMesh = this.buildCollisionMesh();
            this._updateCollisionMesh();
    ...
    _updateCollisionMesh() {
        //
        if (this._collisionMesh == null) return;

        this._collisionMesh.position.copy(this.position);
        this._collisionMesh.quaternion.fromArray(this.rotation);
        this._collisionMesh.scale.copy(this.scale);
        this._collisionMesh.scale.x *= this.geometry.scaleRatio || 1;
    }
...
class InstancedWrapperCollisionMesh extends Mesh {
    //
    constructor(geometry, material, wrapper) {
    ...
    updateWorldMatrix(updateParents, updateChildren) {
        if (updateParents) {
            this.wrapper.updateFromSource();
        } else {
            this.wrapper._updateCollisionMesh();
        }
