...
import InstancedTextData from '../bmfont/instanced'
...
export default class FontMeshWrapper {
    ...
    update( args ){
        ...
        while(i < this.data.sizes.length ) {
            ...
            const wrapper =  this.mesh.add({customFrustumTest: this.customFrustumTest.bind(this)})
    ...
    customFrustumTest( frustum ){
        ...
        if( this.frame != getFrame() ){
            
            try {

                if( this.source ){

                    this.sphere.radius = Math.max( this.size.width, this.size.height ) * ( Math.max(this.source.scale.x, this.source.scale.y, this.source.scale.z))
