...
import { GLOBAL_TEXT_SCALE, DEBUG_BOX } from "../constants";
...
import DebugBoxFactory from "../debugbox";
...
export default class FontWrapper extends Object3D {
    constructor(geo, material, text, options) {
        ...
        this._alpha = this.mesh.userData._alpha = 1;

        // this.raycastMesh = new Mesh(
        //     new BoxGeometry(1, 1, 1),
        // )

        this.mesh.userData.rendermodes = {
            CENTER: MODE.CENTER,
            LEFT: MODE.LEFT,
            TOP: MODE.TOP,
            BOTTOM: MODE.BOTTOM,
    ...
    setAnchor(side, value = true) {
        if (this.mesh.userData.rendermodes[side] == null) {
            console.warn("null font render mode ? ");
        }

        this.mesh.userData.rendermodes[side] = value;
    ...
    setFontSize(val) {
        this.mesh.userData.rendermodes.FONT_SIZE = val;
    }
    ...
    update(opts = {}) {
        ...
        // @ts-ignore - custom geometry method
        this.mesh.geometry.update(
            isNotEmpty ? transformedText : PLACEHOLDER_STYLE.text
        );

        this.mesh.userData._alpha = isNotEmpty ? this.alpha : PLACEHOLDER_STYLE.alpha;

        if (this.mesh.userData.rendermodes.CENTER) {
            this.mesh.geometry.center();
        }

        if (this.mesh.userData.rendermodes.LEFT) {
            this.stickto(LEFT);
        }

        if (opts.verticalalign == "top" || this.mesh.userData.rendermodes.TOP == true) {
            this.stickto(TOP);
        }

        if (this.mesh.userData.rendermodes.RIGHT) {
            this.stickto(RIGHT);
        }

        if (opts.verticalalign == "bottom" || this.mesh.userData.rendermodes.BOTTOM) {
            this.stickto(BOTTOM);
        }

        // if(this.mesh.userData.rendermodes.BILLBOARD ){
        ...
        if (this.mesh.userData.rendermodes.STICK_UNDER != false) {
            let anchor = this.mesh.userData.rendermodes.STICK_UNDER;
            ...
            let height =
                anchor.geometry.boundingBox.max.y -
                anchor.geometry.boundingBox.min.y;
    ...
    resize() {
        ...
        // })
        // // if(this.mesh.userData.rendermodes.FIXED_SIZE){
        // 	let s = (FOV_RATIO_PX.h * this.mesh.userData.rendermodes.FONT_SIZE) / ORIGINAL_FONT_SIZE
    ...
    getLastCharacter() {
        // @ts-ignore - method defined elsewhere
    ...
    renderGlitch() {
        if (this._upGlitch) {
            let progress = Math.min(
                this._glitchCount,
                this._originGlitchText.length
            );

            let str = this.randomString(progress);

            this.update(
                this.replaceWhiteSpaceIndexes(this._whiteSpaceIndexes, str)
            );
            ...
            this.glitchTimer = gsap.delayedCall(
                GLITCH_SPEED,
            ...
            if (this._glitchCount < this._originGlitchText.length + 1) {
                ...
                let glitchStr = this.randomString(
                    this._originGlitchText.length
                );
                ...
                this.update(
                    this.replaceWhiteSpaceIndexes(
                        this._whiteSpaceIndexes,
                        finalStr
                    )
                );
        ...
        // 	let slice = this.text.length - progress

        // 	let str = this._originGlitchText.slice(0, slice)

        // 	str += this.randomString(this._originGlitchText).slice(0, progress)

        // 	this.update(str)
        // }

        //  		else if (this._glitchCount  + 2 < this.text.length ) {

        //  			let length = Math.min(this._glitchCount  + 2, 9)

        //  			this.update(this.randomString(this._originGlitchText).slice(0, length))

        //  		}

        //  		else {

        // 	this.update(this.randomString(this._originGlitchText).slice(0, 9))
        //  		}

        // if (this._glitchCount  < this._glitchCountLimit ) {

        // 	this._glitchCount ++

        // 	this.timer = gsap.delayedCall(GLITCH_SPEED, this.renderGlitch.bind(this))
        // }
