...
import { FBO_DEBUG, DPI } from "../../constants";
...
import { incrementFrame } from "../../utils/frame-counter";
...
class PostProcessing {
  ...
  render(scene, camera, target) {
    // keep this until we finis migration to official three
    ...
    emitter.emit(EngineEvents.BEFORE_SCENE_RENDER, scene, camera);
    ...
    if (
      ...
      if (type == "custom" && this.options?.value?.useOcclusion == true) {
  ...
  renderOcclusionTarget(scene, camera) {
    ...
    emitter.emit(EngineEvents.OCCLUSION, true);
    ...
    emitter.emit(EngineEvents.OCCLUSION, false);
