...
Packer.prototype = {
    ...
    refitWaste: function(block, wasteSpaces) {
        ...
        for (let i = 0; i < wasteSpaces.length && !fitted; i++) {
            ...
            if (block.w <= node.w && block.h <= node.h) {
                ...
                if (node.w - block.w > 0) {
                    wasteSpaces.push({ x: node.x + block.w, y: node.y, w: node.w - block.w, h: block.h });
                }
                if (node.h - block.h > 0) {
                    wasteSpaces.push({ x: node.x, y: node.y + block.h, w: block.w, h: node.h - block.h });
                }
    ...
    refit: function(block) {
        ...
        const tryFit = (node) => {
            let widthIncrease = Math.max(this.maxSize.w, node.x + block.w) - this.maxSize.w;
