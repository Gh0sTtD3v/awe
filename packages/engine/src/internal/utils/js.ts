 ...
 * to use in js files
 */
export function asConst<S extends string>(s: S) {
    return s;
}
...
export function upgradeData(data: any, defData: any) {
    //
    // If data is frozen/non-extensible (e.g. from Immer), clone it first
    ...
    Object.keys(defData).forEach((key) => {
        ...
        if (isObject(val) && isObject(defVal)) {
            //
            const upgraded = upgradeData(val, defVal);
            // If the nested object was cloned, update the reference
            if (upgraded !== val) {
                data[key] = upgraded;
            }
        }
...
export function isComponent3D(obj: any): obj is Component3D {
    //
