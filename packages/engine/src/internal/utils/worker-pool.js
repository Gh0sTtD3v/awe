...
import { WEBWORK_DELAY } from "../constants";
import emitter from "../engine-emitter";
import { EngineEvents } from "../engine-events";

export default class WorkerPool {
  constructor(workerFactory, size = 12, debug = false, opts = null) {
    this.jobs = [];
  ...
  addDebug() {
    ...
    emitter.on(EngineEvents.POST_RENDER, this.postUpdate);
  }
  ...
  nextJob(job, worker, transferrable) {
    ...
    (async function () {
      ...
      try {
        ...
        // ArrayBuffer may already be detached if job was cancelled/reused
