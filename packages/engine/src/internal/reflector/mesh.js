...
import VertexShader from "./vert.glsl.ts";
import FragmentShader from "./frag.glsl.ts";
...
class Reflector extends PipeLineMesh {
  constructor(geometry, options) {
    ...
    this.onBeforeRender = function (renderer, scene, camera) {
      ...
      emitter.emit(EngineEvents.MIRROR, true);

      scope.visible = false;

      renderer.render(scene, virtualCamera);

      emitter.emit(EngineEvents.MIRROR, false);
  ...
  set normalMap(val) {
    ...
    if (val != null) {
      Subsystems.textures.loadTexture(val).then((tex) => {
