...
type Pretty<T> = T extends unknown ? { [K in keyof T]: T[K] } : never;
...
export const generateSoloNavMesh = (
    ...
    const { bbMin, bbMax } = getBoundingBox(positions, indices, bounds);

    //
    // Step 1. Initialize build config.
    //
    navMeshGeneratorConfig = convertConfigUnits(navMeshGeneratorConfig);
    ...
    if (
        !buildContours(
            ...
            Recast.RC_CONTOUR_TESS_WALL_EDGES |
                Recast.RC_CONTOUR_TESS_AREA_EDGES
        )
    ) {
    ...
    navMeshCreateParams.setBoundsMin(bbMin);
    navMeshCreateParams.setBoundsMax(bbMax);
