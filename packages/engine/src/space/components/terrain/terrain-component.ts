...
import GridMaterial from './shaders/grid/diffuse';

import GridMaterialLighting from './shaders/grid/lighting';

import BiplanarMaterial from './shaders/biplanar/diffuse';

import BiplanarLightingMaterial from './shaders/biplanar/lighting';
 ...
 * @public
 *
 * Terrain component that creates configurable ground surfaces in a space. Supports three
 * rendering modes — solid color, tiled texture, or procedural shader (grid or biplanar) —
 * and two geometry shapes: rectangular plane or circular ring.
 *
 * When noise displacement is enabled, simplex noise deforms the terrain geometry to create
 * natural-looking hills and valleys, with configurable seed, frequency, smoothing, and
 * island falloff parameters.
 *
 * The terrain automatically creates a physics mesh collider (fixed rigidbody) so that
 * characters and objects can walk on and collide with the surface. The collider can be
 * configured via the `collider` property in {@link TerrainComponentData}.
 *
 * Configure the terrain via {@link TerrainComponentData} properties passed to
 * {@link ComponentManager.create} or defined in the static scene data.
 *
 * @example
 * ```typescript
 * // Basic terrain with default grid shader
 * const terrain = await space.components.create({
 *   type: "terrain",
 *   position: { x: 0, y: 0, z: 0 },
 *   scale: { x: 1000, y: 150, z: 1000 },
 * });
 *
 * // Terrain with a tiled wood texture
 * const texturedTerrain = await space.components.create({
 *   type: "terrain",
 *   mode: "texture",
 *   textureOpts: { id: "wooden" },
 *   tiles: 30,
 *   color: "#ffffff",
 *   scale: { x: 500, y: 1, z: 500 },
 * });
 *
 * // Terrain with biplanar shader (rock on top, grass on sides)
 * const biplanarTerrain = await space.components.create({
 *   type: "terrain",
 *   mode: "shader",
 *   shader: "biplanar",
 *   textureOpts: { id: "rock" },
 *   textureSideOpts: { id: "grass" },
 *   tiles: 20,
 *   edgeTransition: 5,
 *   smoothAngle: 0.7,
 *   noiseEnabled: true,
 *   scale: { x: 800, y: 200, z: 800 },
 *   seed: 12345,
 *   definition: 150,
 *   noiseDomain: 4,
 * });
 *
 * // Flat solid-color terrain
 * const colorTerrain = await space.components.create({
 *   type: "terrain",
 *   mode: "color",
 *   color: "#3a7d44",
 *   scale: { x: 500, y: 1, z: 500 },
...
export class TerrainComponent extends Component3D<any> {
    ...
    protected async init() {
        if (WEB_WORKER_SUPPORT) {
            this.#worker = new Worker(new URL("./floor.worker.js", import.meta.url));
        }
        ...
        this.mesh.castShadow = this.data.castShadow ?? true;

        this.mesh.receiveShadow = this.data.receiveShadow ?? true;
    ...
    private async _update3D(isProgress = false) {
        ...
        if (this.mesh.diffuseMaterials?.material?.uniforms?.gridColor != null) {
            console.log("update grid color");
            
            this.mesh.diffuseMaterials.material.uniforms.gridColor.value.set(
                this.data.gridColor
            );
            this.mesh.lightingMaterials.material.uniforms.gridColor.value.set(
                this.data.gridColor
            );
        }
    ...
    private async updateTexturing() {
        ...
        console.log("update dat texture");
        ...
        if (this.data.mode == MODES.texture) {
            ...
            // if(this.mesh.diffuseMaterials.material.uniforms.gridColor != null ) {
            //     console.log('update grid color')
            //     this.mesh.diffuseMaterials.material.uniforms.gridColor.value.set( this.data.gridColor );
            //     this.mesh.lightingMaterials.material.uniforms.gridColor.value.set( this.data.gridColor );
            // }

            if (
                this.mesh.diffuseMaterials.material.uniforms.lineWidth != null
            ) {
                this.mesh.diffuseMaterials.material.uniforms.lineWidth.value =
                    this.data.lineWidth;
                this.mesh.lightingMaterials.material.uniforms.lineWidth.value =
                    this.data.lineWidth;
            }
        ...
        if (
            ...
            if (image) {
                if (textureCache[image] == null) {
                    textureCache[image] = {
                        url: image,
                        content: await Subsystems.textures.loadSharedTexture(image),
                        used: 0,
                    };
            ...
            if (image2) {
                if (textureCache[image2] == null) {
                    textureCache[image2] = {
                        content: await Subsystems.textures.loadSharedTexture(image2),
                        used: 0,
                    };
                }
