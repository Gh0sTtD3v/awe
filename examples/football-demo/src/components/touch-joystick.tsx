...
function getJoystickMetrics(): JoystickMetrics {
  if (typeof window === "undefined") {
    return { stickSize: 104, thumbSize: 38, outerInset: 9, innerInset: 18, edgeOffset: 14 };
  }
  if (window.innerWidth <= 390) {
    return { stickSize: 88, thumbSize: 32, outerInset: 7, innerInset: 14, edgeOffset: 12 };
  }
  if (window.innerWidth <= 430) {
    return { stickSize: 96, thumbSize: 36, outerInset: 8, innerInset: 16, edgeOffset: 12 };
  }
  return { stickSize: 112, thumbSize: 40, outerInset: 9, innerInset: 18, edgeOffset: 14 };
}

function applyCardinalAssist(x: number, y: number): JoystickPosition {
  const magnitude = Math.hypot(x, y);
  if (magnitude < CARDINAL_ASSIST_MIN) return { x, y };
...
export function TouchJoystick() {
  ...
  const [thumbPosition, setThumbPosition] = useState<JoystickPosition>({ x: 0, y: 0 });
  ...
  function updateJoystick(clientX: number, clientY: number) {
    ...
    const adjustedDistance =
      normalizedDistance <= DEADZONE ? 0 : (normalizedDistance - DEADZONE) / (1 - DEADZONE);
    ...
    const displayVector = applyCardinalAssist(directionX * displayDistance, directionY * displayDistance);
    const outputVector = applyCardinalAssist(directionX * curvedDistance, -directionY * curvedDistance);
    setThumbPosition({ x: displayVector.x * radius, y: displayVector.y * radius });
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40">
      ...
      >
        <div className="absolute rounded-full border border-dashed border-white/20" style={{ inset: metrics.outerInset }} />
        <div className="absolute rounded-full border border-white/12" style={{ inset: metrics.innerInset }} />
        <div
